...
class Job(Base):
    ...
    apply_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True, comment="投递链接")
    apply_email: Mapped[Optional[str]] = mapped_column(String(256), nullable=True, comment="投递邮箱")
    poster_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True, comment="海报图片 URL")
    source_verified: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False, comment="来源已验证")
    verified_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True, comment="验证时间")
    source_platform: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, comment="来源平台")
    view_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False, comment="浏览次数")
    apply_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False, comment="投递次数")
